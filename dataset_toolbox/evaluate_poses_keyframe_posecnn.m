...
% Generates result .mat file for PoseCNN keyframe results for compparison against SilhoNet
...
feature_symmetric_classes = [13, 16, 19, 20, 21];
...
errors_rotation = zeros(100000, 6);
...
for i = 1:numel(keyframes)
    ...
    for j = 1:numel(gt.cls_indexes)
        ...
        R_adj = SpinCalc('EA123toDCM',rot_offsets(:,cls_index)',.0001,0);
        RT_gt = gt.poses(:, :, j);
        if ismember(cls_index, feature_symmetric_classes)
            RT_gt(1:3, 1:3) = get_viewpoint(RT_gt(1:3, 1:3)*inv(R_adj), cls_index, symmetry)*R_adj;
        end
        ...
        if isempty(roi_index) == 0
            ...
            % pose from network
            Q = result.poses(roi_index, 1:4);
            Q([1,2,3,4]) = Q([2,3,4,1]);
            RT(1:3, 1:3) = inv(SpinCalc('QtoDCM',Q,.0001,0));
            RT(:, 4) = result.poses(roi_index, 5:7);
            if ismember(cls_index, feature_symmetric_classes)
                RT(1:3, 1:3) = get_viewpoint(RT(1:3, 1:3)*inv(R_adj), cls_index, symmetry)*R_adj;
            end
            ...
            % pose with silhonet symmetries
            RT_silhonet = RT;
            RT_silhonet(1:3, 1:3) = get_viewpoint(RT(1:3, 1:3)*inv(R_adj), cls_index, symmetry)*R_adj;
            RT_gt_silhonet = RT_gt;
            RT_gt_silhonet(1:3, 1:3) = get_viewpoint(RT_gt(1:3, 1:3)*inv(R_adj), cls_index, symmetry)*R_adj;
            errors_rotation(count, 6) = re(RT_silhonet(1:3, 1:3), RT_gt_silhonet(1:3, 1:3));

            % pose after ICP refinement
            Q = result.poses_icp(roi_index, 1:4);
            Q([1,2,3,4]) = Q([2,3,4,1]);
            RT(1:3, 1:3) = inv(SpinCalc('QtoDCM',Q,.0001,0));
        ...
        if isempty(roi_index) == 0
            RT = zeros(3, 4);
            Q = result_3DCoordinate.poses(roi_index, 1:4);
            Q([1,2,3,4]) = Q([2,3,4,1]);
            RT(1:3, 1:3) = inv(SpinCalc('QtoDCM',Q,.0001,0));
...
errors_rotation_silhonet = errors_rotation_silhonet(1:count, :);
...
save('results_keyframe.mat', 'distances_sys', 'distances_non', 'errors_rotation', 'errors_rotation_silhonet', 'errors_translation',...
...
function R_new = get_viewpoint(R, class_idx, symmetry)
    ...
    eul = [roll, pitch, yaw]
    R_new = SpinCalc('EA123toDCM',eul,.0001,0);