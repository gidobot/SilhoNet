...
function gen_full_silhouettes
% Generates groundtruth 64x64 occluded and unoccluded silhouette labels for
% YCB-Video dataset.
% Also saves camera viewpont id based on Euler roll and pitch angles and a
% viewpoint rotation angle, which is the Euler yaw angle. But these are not
% used in SilhoNet
...
parfor (seq_id = 0:91)
    gen_seq_silhouettes(seq_id,opt,models,symmetry,rot_offsets);
end
end

function gen_seq_silhouettes(seq_id,opt,models,symmetry,rot_offsets)
    if seq_id >= 60
...
%             pause;
        ...
        filename = fullfile(opt.root, 'data', sprintf('%04d/%06d-segments.mat', seq_id, k));
        save(filename,'mask');
